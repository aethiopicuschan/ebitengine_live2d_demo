...
CubismUserModelExtend::CubismUserModelExtend(const std::string modelDirectoryName, const std::string _currentModelDirectory)
    : CubismUserModel(), _modelJson(NULL), _userTimeSeconds(0.0f), _modelDirName(modelDirectoryName), _currentModelDirectory(_currentModelDirectory), _textureManager(new LAppTextureManager())
{
    // パラメータIDの取得
...
void CubismUserModelExtend::ModelOnUpdate2(int width, int height)
{
    Csm::CubismMatrix44 projection;
    // 念のため単位行列に初期化
    projection.LoadIdentity();
